...
button.addEventListener("click", (event) => {
  ...
  userName ? getGitHubData(userName) : alert("Favor, insira uma usuária válida!");
  input.value = "";
});
...
async function getGitHubData(user) {
  ...
  try {
    ...
    console.error("Requisição inválida", err);
  }
}
...
async function createCard(user) {
  ...
  <img class="icons" src="img/followers.png">
  <p><b>${followers}</b> followers</p>
  <a class="repositories-link"><img class="icons" src="img/repository.png"></a>
...
async function createRepositoriesCards(repositories) {
  ...
  repositories.map((repository) => {
    ...
    <p><img src="./img/star_border.png"> ${repository.stargazers_count}</p>
    </div>
    </div>
    `);
  });
